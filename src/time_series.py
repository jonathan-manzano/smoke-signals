        ...
        with open(LOCATIONS_FILE, "r") as f:
            for line in f:
                ...
                if len(parts) == 4:
                    ...
                    locations.append(
                        {"label": f"{loc_name} (ID: {loc_id})", "value": loc_id}
                    )
...
if (
    predict_net_data_raw is None
    or predict_ambient_data_raw is None
...
try:
    ...
    if num_locations_data != len(locations):
        ...
        locations = [
            {"label": f"Location Index {i}", "value": i}
            for i in range(num_locations_data)
        ]
    ...
    if len(predict_net_data_trunc.shape) == 4:
        ...
        if PRED_LEN > sequence_length:
            raise ValueError(
                f"PRED_LEN ({PRED_LEN}) cannot be greater than sequence length ({sequence_length})"
            )
        ...
        print(
            f"Shape after extracting prediction horizon ({PRED_LEN} steps): {predict_net_pred.shape}"
        )
        ...
        predict_ambient_actual = predict_ambient_pred.reshape(
            num_samples * PRED_LEN, num_locs
        )
        label_actual = label_pred.reshape(num_samples * PRED_LEN, num_locs)

        print(
            f"Shape after reshaping (time*pred_len, locations): {predict_net_actual.shape}"
        )
        ...
        if time_data_trunc.shape[0] == num_samples:
            ...
            raise ValueError(
                f"Time data shape {time_data_trunc.shape} does not match number of samples {num_samples}. Cannot repeat timestamps."
            )

    elif (
        len(predict_net_data_trunc.shape) == 2
    ):  # Assumes (time, location) - Less likely given your output
        print(
            "Warning: Data shape is (time, location). Assuming PRED_LEN=1 and no history dimension."
        )
        predict_net_actual = predict_net_data_trunc
        predict_ambient_actual = predict_ambient_data_trunc
        label_actual = label_data_trunc
        time_data_processed = (
            time_data_trunc  # Assumes time_data_raw is 1D array of timestamps/indices
        )
    else:
        raise ValueError(
            f"Unsupported prediction data shape: {predict_net_data_trunc.shape}. Please adapt the slicing."
        )
    ...
    if time_data_processed.shape[0] != predict_net_actual.shape[0]:
        raise ValueError(
            f"Final timestamp array shape {time_data_processed.shape} incompatible with final prediction shape {predict_net_actual.shape}."
        )
    ...
    df_data = {
        "timestamp": np.repeat(
            time_data_dt, num_locs_final
        ),  # Repeat time for each location
        "location_id": np.tile(
            np.arange(num_locs_final), num_times_final
        ),  # Tile locations for each time step
    ...
    print(
        "Please carefully check the NumPy file structures and adjust the slicing/timestamp logic in the 'Data Structure Adaptation' section."
    )
    exit()
            ...
            [
                ...
                dcc.Checklist(
                    ...
                    options=[
                        ...
                        {
                            "label": "Predicted Net PM2.5 (pm25gnn)",
                            "value": "predicted_net",
                        },
                        {
                            "label": "Predicted Ambient PM2.5 (pm25gnn-ambient)",
                            "value": "predicted_ambient",
                        },
                        {
                            "label": "Estimated Fire-Specific PM2.5",
                            "value": "fire_specific",
                        },
                    ],
...
def update_graph(selected_location_id, start_date, end_date, selected_data):
    ...
    if filtered_df.empty:
        return go.Figure().update_layout(
            title=f"No data available for Location ID {selected_location_id} in the selected date range"
        )

    # Create figure
    fig = go.Figure()
    location_label = next(
        (loc["label"] for loc in locations if loc["value"] == selected_location_id),
        f"Location Index {selected_location_id}",
    )
    ...
    if "predicted_net" in selected_data:
        fig.add_trace(
            go.Scatter(
                x=filtered_df["timestamp"],
                y=filtered_df["predicted_net"],
                mode="lines",
                name="Predicted Net (pm25gnn)",
                line=dict(color="red"),
            )
        )
    if "predicted_ambient" in selected_data:
        fig.add_trace(
            go.Scatter(
                x=filtered_df["timestamp"],
                y=filtered_df["predicted_ambient"],
                mode="lines",
                name="Predicted Ambient (pm25gnn-ambient)",
    ...
    fig.update_layout(
        ...
        xaxis_range=[
            start_datetime,
            end_datetime,
        ],  # Ensure plot respects date picker range
    )
